...
Write-Host "🚀 Démarrage du frontend LinkUp..." -ForegroundColor Green
...
Set-Location linkup-frontend

# Vérifier si package.json existe
if (Test-Path "package.json") {
    Write-Host "✅ package.json trouvé" -ForegroundColor Green
} else {
    Write-Host "❌ package.json non trouvé dans le dossier linkup-frontend" -ForegroundColor Red
    exit 1
}
