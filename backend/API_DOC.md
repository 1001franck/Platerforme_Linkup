...
- POST /forgotten-password/mail
- POST /forgotten-password/update
...
```bash
curl -H "Cookie: token=$TOKEN" http://localhost:3000/users/me
```
...
```bash
curl -X DELETE -H "Cookie: token=$TOKEN" http://localhost:3000/users/me
```

## Mot de passe

### Mot de passe oublié - Envoi d'un mail pour réinitialiser :

```bash
curl -X POST http://localhost:3000/forgotten-password/mail \
	-H "Content-Type: application/json" \
	-d '{"email":"email@gmail.com"}'
```

### Mot de passe oublié - Modifier le mot de passe en bdd :

<!-- Le token à inscrire est celui dans le lien de la page pour réinitialiser le mdp -->
<!-- C'est un token spécial qui dure 15 min, ce n'est pas celui de connexion -->

```bash
	curl -X POST http://localhost:3000/forgotten-password/reset \
	-H "Content-Type: application/json" \
	-d '{
...
### Modifier son mot de passe - Vérifier que le mot de passe entré est le même qu'en bdd

```bash
	curl -X POST http://localhost:3000/reset-password/verify \
	-H "Content-Type: application/json" \
	-d '{"currentPassword":"oldpassword"}'
```

### Modifier son mot de passe - Modifier le mot de passe inscrit en bdd

```bash
curl -X PUT http://localhost:3000/reset-password/update \
 -H "Content-Type: application/json" \
...
```bash
curl -X DELETE -H "Cookie: token=$TOKEN" http://localhost:3000/jobs/1
```
...
```bash
curl -X DELETE -H "Cookie: token=$TOKEN" http://localhost:3000/companies/me
```
...
```bash
curl -H "Cookie: token=$TOKEN" http://localhost:3000/applications/my
```

### Stats (admin) :

```bash
curl -H "Cookie: token=$TOKEN" http://localhost:3000/applications/stats
```
...
```bash
curl -H "Cookie: token=$TOKEN" http://localhost:3000/messages/conversations
```

### Messages avec un user :

```bash
curl -H "Cookie: token=$TOKEN" http://localhost:3000/messages/2
```

### Marquer comme lu :

```bash
curl -X PUT -H "Cookie: token=$TOKEN" http://localhost:3000/messages/123/read
```
...
```bash
curl -H "Cookie: token=$TOKEN" http://localhost:3000/saved-jobs
```

### Supprimer sauvegarde :

```bash
curl -X DELETE -H "Cookie: token=$TOKEN" http://localhost:3000/saved-jobs/1
```
...
```bash
curl -H "Cookie: token=$TOKEN" http://localhost:3000/user-files/me
```

### Supprimer un fichier :

```bash
curl -X DELETE -H "Cookie: token=$TOKEN" http://localhost:3000/user-files/12
```
...
```bash
curl -H "Cookie: token=$TOKEN" http://localhost:3000/admin/dashboard
```

### Ban user :

```bash
curl -X POST -H "Content-Type: application/json" -H "Cookie: token=$TOKEN" http://localhost:3000/admin/users/4/ban -d '{"reason":"spam","duration":"7d"}'
```
